#returns for each time interval the conversion rate between the RON and each given currency in the query
...
for (i in 1:length(a))
{
  tmp<-getFX(paste("RON/",a[i],sep = ""),
             from=from,
             to=to,
             auto.assign=FALSE)
  colnames(tmp)<-gsub("RON.","",colnames(tmp))
